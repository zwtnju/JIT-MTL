...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer, RobertaModel, PLBartConfig,
                          PLBartTokenizer, PLBartForConditionalGeneration, T5Config,
                          T5ForConditionalGeneration)
from tqdm import tqdm
from multi_task_model import Model, MixModel, MMTModel
from util import TextDataset, eval_result, preprocess_code_line, get_line_level_metrics, create_path_if_not_exist
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (_, input_ids, manual_features, label, line_label) \
                = [x.to(args.device) for x in batch]
            model.train()
            loss, _, _ = model(input_ids, manual_features, label, line_label)
            if args.n_gpu > 1:
                loss = loss.mean()
...
def evaluate(args, model, tokenizer, eval_when_training=False, eval_dataset=None):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
        (commit_idx, input_ids, manual_features, label, line_label) \
            = [x.to(args.device) for x in batch]
        with torch.no_grad():
            loss, logit, dl_logit = model(input_ids, manual_features, label, line_label)
            torch.cuda.empty_cache()
            eval_loss += loss.mean().item()
            commit_idxs.append(commit_idx.cpu().numpy())
            logits.append(logit.cpu().numpy())
            y_trues.append(label.cpu().numpy())
            line_logits.append(dl_logit[:, 1].cpu().numpy())
        nb_eval_steps += 1
    ...
    # aggregation of commit results
    result_df = pd.DataFrame({'commit_idxs': commit_idxs, 'y_preds': y_preds, 'y_prob': logits[:, 1],
                              'y_trues': y_trues, 'line_logits': line_logits})
    aggregate_result_df = result_df.groupby(['commit_idxs']).mean()
    aggregate_result_df = aggregate_result_df.reset_index()
    aggregate_result_df['y_preds'] = aggregate_result_df['y_preds'].apply(lambda x: 0 if x < 0.5 else 1)
    commit_ids = []
    commit_tokens = []
    ...
    aggregate_result_df['commit_ids'] = commit_ids
    y_preds = aggregate_result_df['y_preds'].to_numpy()
    y_trues = aggregate_result_df['y_trues'].to_numpy()
...
def test(args, model, tokenizer, best_threshold=0.5, eval_dataset=None):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
        (commit_idx, input_ids, manual_features, label, line_label) \
            = [x.to(args.device) for x in batch]
        with torch.no_grad():
            loss, dp_logit, dl_logit = model(input_ids, manual_features, label, line_label, True)
            eval_loss += loss.mean().item()
            commit_idxs.append(commit_idx.cpu().numpy())
            logits.append(dp_logit.cpu().numpy())
            y_trues.append(label.cpu().numpy())
            line_logits.append(dl_logit[:, 1].cpu().numpy())
    ...
    line_commit_ids, line_idxs, line_labels, line_probs = [], [], [], []

    for commit_id, pred, label, input_token in zip(commit_ids, y_preds, y_trues, commit_tokens):
        # calculate
        if int(label) == 1 and int(pred) == 1 and '[ADD]' in input_token:
            ...
            cur_commit = aggregate_result_df[aggregate_result_df['commit_ids'] == commit_id]
            commit_idx = int(cur_commit['commit_idxs'].iloc[0])
            cur_line_sc = result_df[result_df['commit_idxs'] == commit_idx]['line_logits']
            cur_line_sc = cur_line_sc.tolist()

            assert code_line_nums == len(cur_line_sc), \
                'code lines does not match, commit_idxs: {}, idx: {}'.format(len(cur_line_sc), code_line_nums)
            ...
            cur_commit_id = [commit_id] * code_line_nums
            line_commit_ids.extend(cur_commit_id)
            line_idxs.extend(cur_labels['idx'].to_list())
            line_labels.extend(cur_labels['label'].to_list())
            line_probs.extend(cur_line_sc['line_score'].to_list())
    ...
    RF_result = pd.DataFrame(aggregate_result_df, columns=['commit_ids', 'y_prob', 'y_preds', 'y_trues'])
    RF_result.to_csv(os.path.join(args.output_dir, "predictions.csv"), sep='\t', index=None)

    line_preds = list(map(lambda x: 1 if x > best_threshold else 0, line_probs))
    line_result_df = pd.DataFrame({'commit_ids': line_commit_ids, 'line_idxs': line_idxs, 'line_preds': line_preds,
                                   'line_labels': line_labels, 'line_probs': line_probs})

    line_result_df.to_csv(os.path.join(args.output_dir, "line_predictions.csv"))
    RF_result.to_csv(os.path.join(args.output_dir, "commit_predictions.csv"))
...
def parse_args():
    ...
    parser.add_argument("--mix_checkpoint", action='store_true',
                        help="Whether to load defect code representation.")
    parser.add_argument("--defect_code_representation_checkpoint", default=None, type=str,
                        help="The defect code representation model checkpoint to be fine-tuned.")
    parser.add_argument("--run_expert_feature", action='store_true',
                        help="Whether to use expert feature.")
...
def main(args):
    ...
    mtl_model = Model(model, config, tokenizer, args)
    # Training
    if args.do_train:
        if args.semantic_checkpoint:
            ...
            mtl_model.load_state_dict(checkpoint['model_state_dict'], strict=False)
        ...
        if args.mix_checkpoint:
            output_dir = args.defect_code_representation_checkpoint
            logger.info("Loading defect code representation checkpoint from {}".format(output_dir))
            checkpoint = torch.load(output_dir)
            mix_model = MixModel(model, config, tokenizer, args)
            mix_model.load_state_dict(checkpoint['model_state_dict'])
            mtl_model = MMTModel(mix_model)
            mtl_model.to(args.device)
        ...
        train(args, train_dataset, mtl_model, tokenizer, eval_dataset)
    ...
    if args.do_test:
        ...
        test(args, mtl_model, tokenizer, best_threshold=0.5, eval_dataset=test_dataset)
